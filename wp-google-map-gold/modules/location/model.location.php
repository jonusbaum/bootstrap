				...
				if ( isset( $_FILES['import_file']['tmp_name'] ) and '' == sanitize_file_name( wp_unslash( $_FILES['import_file']['tmp_name'] ) ) ) {
					...
					if ( ! empty( $file_data ) ) {
						...
						foreach ( $file_data as $data ) {
							...
							if( $datas['location_longitude'] != '' ) {
								$datas['location_longitude'] = (float) filter_var( $datas['location_longitude'], FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION );
							}

							if( $datas['location_latitude'] != '' ) {
								$datas['location_latitude'] = (float) filter_var( $datas['location_latitude'], FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION );
							}
							...
							$entityID = '';
							if ( isset( $data['id'] ) ) {
								$entityID = intval( wp_unslash( $data['id'] ) );
							}
